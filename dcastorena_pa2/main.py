#Project Assignment 2 (Main File)
...
while (userQuery != ".exit"):
  ...
  #create a database
    ...
    if dbutils.checkDatabase(dbName) == 0:
  ...
  #delete a database
    ...
    if dbutils.checkDatabase(dbName):
      os.system(f'rm -r {dbName}')
      print(f"Database {dbName} removed.")
    else:
  ...
  #sets currently active database
    ...
    if dbutils.checkDatabase(workingDB):
  ...
  #creates a table with specified name and attributes
    ...
    if (workingDB != None):
      if dbutils.checkTable(tName, workingDB) == 0:
